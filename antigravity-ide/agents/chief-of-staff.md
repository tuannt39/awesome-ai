...
- **Rules are system-injected**: `.gemini/antigravity-ide/rules/*.md` files load automatically every session. Unlike prompt instructions, the LLM cannot choose to ignore them.

## Example Invocations

```bash
Antigravity /mail                    # Email-only triage
Antigravity /slack                   # Slack-only triage
Antigravity /today                   # All channels + calendar + todo
Antigravity /schedule-reply "Reply to Sarah about the board meeting"
```

## Prerequisites

- [Antigravity CLI](https://docs.anthropic.com/en/docs/Antigravity-code)
