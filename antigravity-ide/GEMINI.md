...
2. **Discovery Agents**: Scan agent directories in priority order:
   - **Project-level** (high priority): `<project-root>/antigravity-ide/agents/` — project-specific agents
   - **Global-level** (fallback): `~/.gemini/antigravity-ide/agents/` — shared agents across all projects
...
3. **Discovery Rules**: Scan rule directories in priority order:
   - **Project-level** (high priority): `<project-root>/antigravity-ide/rules/` — project-specific rules
   - **Global-level** (fallback): `~/.gemini/antigravity-ide/rules/` — shared rules across all projects
...
- **Subagent Discovery** (already scanned during Bootstrap — Section 0):
  - Look up agent definitions in order: **project-level** (`<project-root>/antigravity-ide/agents/`) → **global-level** (`~/.gemini/antigravity-ide/agents/`).
...
## 8. Rules System
- **Rules Directory**: `~/.gemini/antigravity-ide/rules` contains coding guidelines structured hierarchically:
