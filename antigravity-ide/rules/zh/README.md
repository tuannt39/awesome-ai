...
```bash
# 创建目标目录
mkdir -p ~/.gemini/antigravity-ide/rules

# 安装通用规则（所有项目必需）
cp -r rules/common ~/.gemini/antigravity-ide/rules/common

# 安装中文翻译版本（可选）
cp -r rules/zh ~/.gemini/antigravity-ide/rules/zh

# 根据项目技术栈安装语言特定规则
cp -r rules/typescript ~/.gemini/antigravity-ide/rules/typescript
cp -r rules/python ~/.gemini/antigravity-ide/rules/python
cp -r rules/golang ~/.gemini/antigravity-ide/rules/golang
cp -r rules/swift ~/.gemini/antigravity-ide/rules/swift
cp -r rules/php ~/.gemini/antigravity-ide/rules/php
```
