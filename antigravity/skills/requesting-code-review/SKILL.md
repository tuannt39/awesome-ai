...
Use the Antigravity CLI subagent definition and invocation mechanism (`invoke_subagent` with `TypeName` as `research` or `self`), using the template at `~/.gemini/antigravity/skills/requesting-code-review/code-reviewer.md`.
...
[Trigger reviewer subagent via Antigravity CLI]
  ...
  PLAN_OR_REQUIREMENTS: Task 2 in the plan file ~/.gemini/antigravity/brain/plans/deployment-plan.md
  BASE_SHA: a7981ec
  HEAD_SHA: 3df7661
...
See the detailed configuration template at: `~/.gemini/antigravity/skills/requesting-code-review/code-reviewer.md`
